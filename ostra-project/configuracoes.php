...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_profile') {
    ...
    if (!empty($new_password)) {
        ...
        if (empty($_POST['currentPassword'])) {
            $update_error = "Você precisa informar sua senha atual para alterar a senha.";
        } else {

            // Buscar senha atual do banco
            $stmt_pass = $conn->prepare("SELECT ds_senha FROM usuarios WHERE id_usuario = ?");
            $stmt_pass->bind_param("i", $usuario_id);
            $stmt_pass->execute();
            $senha_bd = $stmt_pass->get_result()->fetch_assoc()['ds_senha'];
            $stmt_pass->close();

            // Validar senha atual
            if (!password_verify($_POST['currentPassword'], $senha_bd)) {
                $update_error = "Senha atual incorreta.";
            }
        }

        // 2. Validar nova senha e confirmação
        if (empty($update_error)) {
...
<header class="top-header">
    <div class="search-container">
        <svg class="search-icon" width="20" height="20"><circle cx="11" cy="11" r="8"/><path d="M21 21L16.65 16.65"/></svg>
        <input type="text" class="search-input" placeholder="Search Bar . . .">
    </div>

    <div class="user-profile">
        <div class="user-avatar">
            <svg width="32" height="32"><circle cx="16" cy="16" r="16"/><circle cx="16" cy="12" r="5"/><path d="M6 26C6 21 10 18 16 18C22 18 26 21 26 26"/></svg>
        </div>
    </div>
</header>
...
<div class="config-container">
    ...
    <div class="config-content-area-A">
        ...
        <?php if (!isset($_GET['tab']) || $_GET['tab'] == 'perfil'): ?>
            ...
            <form action="configuracoes.php" method="POST" class="config-form">
                ...
                <div class="form-group-A">
                    ...
                    <input class="Inputa" type="password" id="currentPassword" name="currentPassword">
                </div>
        ...
        <?php elseif ($_GET['tab'] == 'midia'): ?>
            ...
            <p class="config-subtitle">O upload de fotos é feito diretamente na <a href="profile.php">página de perfil</a>.</p>
